...
const fetchInquiries = async () => {
  try {
    ...
    // 페이지네이션 설정
    inquiryPagination.value.pagesNumber = Math.ceil(
      response.data.data.content.length / inquiryPagination.value.rowsPerPage
    );
    inquiryPagination.value.isFirstPage = inquiryPagination.value.page === 1;
    inquiryPagination.value.isLastPage =
      inquiryPagination.value.page === inquiryPagination.value.pagesNumber;
  } catch (error) {
