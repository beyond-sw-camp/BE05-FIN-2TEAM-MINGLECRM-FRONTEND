      ...
      <template v-slot:body="props">
        <q-tr :props="props" @click="showPaymentDetail(props.row)">
          <q-td v-for="col in paymentColumns" :key="col.name" :props="props">
            <span
              v-if="shouldHighlight(props.row[col.field])"
...
const fetchPayments = async () => {
  try {
    ...
    // 페이지네이션 설정
    paymentPagination.value.pagesNumber = Math.ceil(
      payments.value.length / paymentPagination.value.rowsPerPage
    );
    paymentPagination.value.isFirstPage = paymentPagination.value.page === 1;
    paymentPagination.value.isLastPage =
      paymentPagination.value.page === paymentPagination.value.pagesNumber;
