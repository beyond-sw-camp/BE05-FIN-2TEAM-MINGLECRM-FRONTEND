    ...
    >
      <template v-slot:body="props">
        <q-tr :props="props" @click="showReservationDetail(props)">
          <q-td
            v-for="col in hotelReservationColumns"
            :key="col.name"
            :props="props"
          >
            {{ props.row[col.field] }}
...
const fetchReservations = async () => {
  try {
    const response = await api.get(
      `/api/v1/customers/${customerId}/hotel/reservations`
    );
    ...
    // 페이지네이션 설정
    reservationPagination.value.pagesNumber = Math.ceil(
      reservations.value.length / reservationPagination.value.rowsPerPage
    );
    reservationPagination.value.isFirstPage =
      reservationPagination.value.page === 1;
    reservationPagination.value.isLastPage =
      reservationPagination.value.page ===
      reservationPagination.value.pagesNumber;
