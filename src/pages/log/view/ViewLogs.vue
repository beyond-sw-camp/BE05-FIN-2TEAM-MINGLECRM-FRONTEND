                  ...
                  <q-icon name="event" class="cursor-pointer">
                    ...
                    >
                      <q-date
                        v-model="endDate"
                        mask="YYYY-MM-DD"
                        @update:model-value="setEndDateToEndOfDay"
                      >
                        <div class="row items-center justify-end">
                          <q-btn
                            v-close-popup
    ...
    <div class="table-container">
      ...
      >
        ...
        <template #body-cell-logs="props">
          <q-td :props="props">
            <q-expansion-item
              v-for="(log, index) in props.row.logsByDate"
              :key="index"
              :label="`${log.date} (${log.times.length} 조회)`"
            >
              <q-list>
                <q-item-label v-for="(time, idx) in log.times" :key="idx">{{
                  time
                }}</q-item-label>
              </q-list>
            </q-expansion-item>
...
export default {
  setup() {
    ...
    const fetchData = () => {
      const params = {};
      if (search.value.employeeName)
        params.employeeName = search.value.employeeName;
      if (search.value.employeeEmail)
        params.employeeEmail = search.value.employeeEmail;
      if (search.value.employeeGrade)
        params.employeeGrade =
          search.value.employeeGrade?.value || search.value.employeeGrade;
      if (search.value.customerName)
        params.customerName = search.value.customerName;
      if (search.value.customerEmail)
        params.customerEmail = search.value.customerEmail;
      if (search.value.customerGrade)
        params.customerGrade =
          search.value.customerGrade?.value || search.value.customerGrade;
      if (search.value.startDate)
        params.startDate = new Date(search.value.startDate).toISOString(); // 시작 날짜를 ISO 형식으로 변환하여 전송
      if (search.value.endDate)
        params.endDate = new Date(search.value.endDate).toISOString(); // 종료 날짜를 ISO 형식으로 변환하여 전송

      axios
        .get("https://d14sl6ndoksytf.cloudfront.net/api/v1/view-logs/search", {
          params,
        })
        .then((response) => {
    ...
    const groupLogs = () => {
      ...
      viewLogs.value.forEach((log) => {
        ...
        let dateEntry = grouped[key].logsByDate.find(
    ...
    onMounted(() => {
      axios
        .get("https://d14sl6ndoksytf.cloudfront.net/api/v1/view-logs")
        .then((response) => {
