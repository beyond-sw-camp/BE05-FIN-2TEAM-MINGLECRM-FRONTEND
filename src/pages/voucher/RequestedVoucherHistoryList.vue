...
const fetchVouchers = async () => {
  try {
    const response = await axios.get(
      "https://mingle-crm.com/api/v1/vouchers/requested"
    );
...
const approveVoucher = async (voucherId) => {
  Dialog.create({
    ...
    try {
      await axios.post(
        `https://mingle-crm.com/api/v1/vouchers/approval/${voucherId}`
      );
      Notify.create({
        type: "positive",
...
const rejectVoucher = async (voucherId) => {
  Dialog.create({
    ...
    try {
      await axios.post(
        `https://mingle-crm.com/api/v1/vouchers/rejection/${voucherId}`,
        { reason }
      );
      Notify.create({
        type: "positive",
