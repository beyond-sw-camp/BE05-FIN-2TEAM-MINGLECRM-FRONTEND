<template>
  <div>
    <h2 class="text-h6">검토 전 바우처 목록</h2>
...
const fetchVouchers = async () => {
  try {
    const response = await axios.get(
      `http://15.165.109.152:8080/api/v1/vouchers/requested-marketer`
    );
    vouchers.value = response.data.data;
    errorMessage.value = "";
  } catch (error) {
    console.error("검토 전 바우처 목록을 불러오는 중 에러 발생:", error);
    errorMessage.value =
      "검토 전 바우처 목록을 불러오는 중 에러가 발생했습니다.";
  } finally {
    loading.value = false;
  }
};
